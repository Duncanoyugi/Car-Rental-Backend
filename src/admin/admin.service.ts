...
import { User, Role, AgentRequestStatus } from '../../generated/prisma';
...
export class AdminService {
  ...
  async getAgentRequests(status?: string) {
    return this.prisma.agentRequest.findMany({
      where: status ? { status: status as AgentRequestStatus } : undefined,
      include: { user: true },
      orderBy: { createdAt: 'desc' },
    });
  }

  async approveAgentRequest(id: string) {
  ...
  async rejectAgentRequest(id: string, reason?: string) {
    const request = await this.prisma.agentRequest.findUnique({
      where: { id },
      include: { user: true },
    });
    ...
    await this.mailService.sendMail({
      to: request.user.email,
      subject: 'Your Agent Request Status',
