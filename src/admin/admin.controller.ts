...
export class AdminController {
  ...
  getAgentRequests(@Query('status') status?: string) {
    return this.adminService.getAgentRequests(status);
  }

  @Patch('agent-requests/:id/approve')
  approveAgentRequest(@Param('id') id: string) {
    return this.adminService.approveAgentRequest(id);
  }

  @Patch('agent-requests/:id/reject')
  rejectAgentRequest(@Param('id') id: string, @Body('reason') reason?: string) {
    return this.adminService.rejectAgentRequest(id, reason);
  }
}
